...
USER root
# procps supplies `ps`, which Nextflow uses for per-task resource metrics;
...
# GATK spawns a JVM per task. Without a cap, it sizes the heap from the host's
# total RAM and gets OOM-killed inside a memory-limited container.
ENV JAVA_TOOL_OPTIONS="-XX:+UseSerialGC"

# Include Nextflow in the image so the container is self-sufficient: the
...
# WFABC (Foll et al. 2015) is not packaged by conda, so it is
...
# Install IRMA from the CDC's release. Bioconda stops at 1.0.3; the releases carry
...
# Install SNPGenie from the author's source rather than bioconda. The conda
...
# Pin the source to a commit so a rebuild cannot change the analysis silently.
...
# Copy the pipeline last because it changes more often than the
...
# Record when this image was built. The version string alone cannot distinguish
...
# FLUMINA_CONTAINER tells the launcher that it is already inside the image, so it
# defaults to the `standard` profile and runs tasks in this container rather
# than trying to start a nested one.
#
# NXF_HOME points to the pre-warmed framework installed above. NXF_OFFLINE stops
