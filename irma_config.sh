...
# directly from the value ("$TMP"/user/IRMAvX/run-token), so the path must
# already exist. If it does not, IRMA produces empty output and still exits 0.
#
...
# LOCAL_PROCS_OVERRIDE from the CPUs the scheduler granted, so IRMA matches the
# allocation. Setting these manually can oversubscribe a node or leave part of
# the allocation idle.
#SINGLE_LOCAL_PROC=2
#DOUBLE_LOCAL_PROC=1
